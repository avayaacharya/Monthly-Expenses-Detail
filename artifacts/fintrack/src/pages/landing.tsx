...
import { Cloud, Code2, BarChart3, Shield, Heart, BookOpen, Laptop, Wallet, TrendingUp, FileText, Lightbulb, ChevronRight, Star } from "lucide-react";
    ...
    <div style={{ fontFamily: "'Inter', sans-serif", color: "#0F1E3C", background: "#fff" }}>
      <nav style={{ position: "sticky", top: 0, zIndex: 100, background: "rgba(255,255,255,0.95)", backdropFilter: "blur(12px)", borderBottom: "1px solid #D1E3FF", padding: "0 32px", height: 64, display: "flex", alignItems: "center", justifyContent: "space-between", boxShadow: "0 1px 12px rgba(26,79,186,0.06)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <span style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", fontWeight: 700, fontSize: 18, color: "#0F1E3C" }}>
            Acharya Technology <span style={{ color: "#64748B", fontWeight: 400, margin: "0 6px" }}>|</span> <span style={{ color: "#1A4FBA" }}>FinTrack</span>
          </span>
        </div>
        <button data-testid="btn-employee-login" onClick={() => setLocation("/login")}
          style={{ background: "linear-gradient(90deg, #1A4FBA, #2A7BDE)", color: "#fff", border: "none", borderRadius: 10, padding: "10px 24px", fontFamily: "'Inter', sans-serif", fontWeight: 600, fontSize: 14, cursor: "pointer", boxShadow: "0 4px 12px rgba(26,79,186,0.25)" }}>
          Employee Login
        </button>
      </nav>

      <section ref={heroRef} style={{ minHeight: "100vh", background: "linear-gradient(160deg, #0A1628 0%, #0F1E3C 40%, #1A3A80 70%, #1A4FBA 100%)", display: "flex", alignItems: "center", justifyContent: "center", position: "relative", overflow: "hidden", padding: "80px 32px" }}>
        ...
        <div style={{ position: "relative", textAlign: "center", maxWidth: 720, transform: `translateY(${scrollY * 0.15}px)` }}>
          ...
          <h1 style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", fontSize: "clamp(36px, 6vw, 56px)", fontWeight: 800, color: "#fff", lineHeight: 1.1, marginBottom: 24, letterSpacing: "-1px" }}>
          ...
          <p style={{ fontSize: 20, color: "rgba(255,255,255,0.7)", marginBottom: 12, fontWeight: 400 }}>Your Personal Finance Companion</p>
          <p style={{ fontSize: 15, color: "rgba(255,255,255,0.5)", marginBottom: 48 }}>
            ...
            <span onClick={() => setLocation("/login")} style={{ color: "#0EA5E9", cursor: "pointer", textDecoration: "underline" }}>Sign in to your account</span>
          </p>
          <button data-testid="btn-hero-login" onClick={() => setLocation("/login")}
            style={{ background: "linear-gradient(90deg, #0EA5E9, #2A7BDE)", color: "#fff", border: "none", borderRadius: 14, padding: "18px 48px", fontSize: 18, fontWeight: 700, fontFamily: "'Plus Jakarta Sans', sans-serif", cursor: "pointer", boxShadow: "0 8px 32px rgba(14,165,233,0.35)", display: "inline-flex", alignItems: "center", gap: 10 }}>
          ...
          <div style={{ marginTop: 64, display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 24, textAlign: "center" }}>
            {[{ v: "1,200+", l: "Active Employees" }, { v: "6", l: "Office Locations" }, { v: fmt(24000000), l: "Savings Tracked" }].map(s => (
              <div key={s.l} style={{ background: "rgba(255,255,255,0.07)", borderRadius: 16, padding: 20, border: "1px solid rgba(255,255,255,0.1)" }}>
      ...
      <section id="services" data-animate style={{ padding: "96px 32px", background: "#fff", opacity: visible["services"] ? 1 : 0, transform: visible["services"] ? "none" : "translateY(24px)", transition: "all 0.6s ease 0.1s" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 24 }}>
            {services.map(s => (
              <div key={s.title} style={{ background: "rgba(255,255,255,0.75)", backdropFilter: "blur(10px)", border: "1px solid rgba(209,227,255,0.6)", boxShadow: "0 4px 24px rgba(26,79,186,0.08)", borderRadius: 20, padding: 32, transition: "transform 0.2s, box-shadow 0.2s" }}
      ...
      <section id="why" data-animate style={{ padding: "96px 32px", background: "#fff", opacity: visible["why"] ? 1 : 0, transform: visible["why"] ? "none" : "translateY(24px)", transition: "all 0.6s ease" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: 32 }}>
            {whyItems.map((w, i) => (
              <div key={w.title} style={{ textAlign: "center", padding: "40px 32px", borderRadius: 24, background: i === 1 ? "linear-gradient(135deg, #EBF3FF, #D1E8FF)" : "#F5F8FF", border: `1px solid ${i === 1 ? "#D1E3FF" : "#E8F0FF"}`, transition: "transform 0.2s" }}
                onMouseEnter={e => (e.currentTarget as HTMLDivElement).style.transform = "translateY(-3px)"}
                onMouseLeave={e => (e.currentTarget as HTMLDivElement).style.transform = "none"}>
      ...
      <section style={{ padding: "80px 32px", background: "linear-gradient(90deg, #1A4FBA, #2A7BDE)", textAlign: "center" }}>
        ...
        <button data-testid="btn-cta-login" onClick={() => setLocation("/login")}
          style={{ background: "#fff", color: "#1A4FBA", border: "none", borderRadius: 12, padding: "16px 40px", fontSize: 17, fontWeight: 700, fontFamily: "'Plus Jakarta Sans', sans-serif", cursor: "pointer", boxShadow: "0 8px 24px rgba(0,0,0,0.15)" }}>
          Employee Login
        </button>
      </section>
      ...
      <footer style={{ background: "#0F1E3C", padding: "48px 32px 32px", color: "rgba(255,255,255,0.6)" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 40, flexWrap: "wrap", gap: 24 }}>
            ...
            <div style={{ display: "flex", gap: 32 }}>
              {["Home", "Employee Login", "Contact HR"].map(link => (
                <span key={link} onClick={link === "Employee Login" ? () => setLocation("/login") : undefined}
                  style={{ fontSize: 14, cursor: link === "Employee Login" ? "pointer" : "default", transition: "color 0.2s" }}
          ...
          <div style={{ borderTop: "1px solid rgba(255,255,255,0.1)", paddingTop: 24, fontSize: 13, textAlign: "center" }}>
            © {new Date().getFullYear()} Acharya Technology. All rights reserved.
          </div>
        </div>
      </footer>
    </div>
